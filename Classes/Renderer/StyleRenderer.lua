...
function StyleRenderer:renderStyle()
			...
			if not file then
			  ...
			  while true do
				...
				if string.find(sentence,class) then
				  sentence = sentence:gsub(class..'_','')
				  ...
				  leftButton = Widget.newButton{
					...
					x = xOfButton,
					y = yOfButton,
					defaultFile = 'Assets/Buttons/left.png',
					overFile = 'Assets/Buttons/left.png',
					onEvent = function( event )
					if event.phase == 'ended' then
					  ...
					  if currentItem[itemIndex] ~= nil then
						table.remove(currentItem,event.target.id) end
				  ...
				  rightButton = Widget.newButton{
					...
					x = xOfButton2,
					y = yOfButton,
					defaultFile = 'Assets/Buttons/right.png',
					overFile = 'Assets/Buttons/right.png',
					onEvent = function( event )
					if event.phase == 'ended' then
					  tryIt(event.target.id)
					  itemIndex = itemIndex + 1
	...
	local leftMe = display.newImageRect(chooseGroup,'Assets/Buttons/left.png',40,40)
	leftMe.x = display.contentWidth * .05
	leftMe.y = display.contentHeight * .055
	leftMe.xScale = .5
	local rightMe = display.newImageRect(chooseGroup,'Assets/Buttons/right.png',40,40)
